...
for dataset in set(datasets) - {'benchmark', 'spambase_processed'}:
...
data = {columns[1]: [], columns[2]: [], columns[3]: [], "Selected Features": []}
dt_with_path = [f for f in glob(os.path.join(results_path, 'spambase_processed/*.csv')) if 'summary.csv' not in f]
dt_without_path = [f for f in glob(os.path.join(results_path, 'spambase/*.csv')) if 'summary.csv' not in f]
...
for path in dt_without_path:
    ...
    data[columns[2]] += [np.std(scores.flatten())]
    data[columns[3]] += [int(path.split('/')[-1][:-4])]
    data["Selected Features"] += ['No']

data = DataFrame(data)

ax = sns.scatterplot(x=columns[3], y=columns[1], hue="Selected Features", size=columns[2], data=data, palette='winter')
ax.legend(loc='lower right')
# ax.set_title(dataset)

graphic_path = os.path.join(graphics_path, 'spambase.pdf')
plt.savefig(graphic_path, dpi=360, pad_inches=0.1, bbox_inches='tight')
plt.close()